...
ClassificationNN::ClassificationNN(const topologyType& topology, const inputsType& inputs,
						const outputsType& outputs,std::string activationFunction) :
...
ClassificationNN::ClassificationNN(const topologyType& topology, const std::string& fileName, int numberOfCollums,
						int indexOfClassCollumn, std::string activationFunction) :
						NeuralNetwork(topology, activationFunction) {
	readDataset(fileName, numberOfCollums, indexOfClassCollumn);
	normalizeInputs();
}

void ClassificationNN::readDataset(const std::string& fileName, int numberOfCollums, int outputCollum){
...
void ClassificationNN::trainAndTest(int epochs = 1000) {
    ...
    for (int epoch = 0; epoch < epochs; ++epoch) {
        ...
        for (size_t i = 0; i < trainingInputs.size(); ++i) {
            ...
            backpropagate(trainingOutputs[i]);  // Assuming your NN has a backprop method
        }
...
inputsType ClassificationNN::getInputs() const {
	return inputs;
}

outputsType ClassificationNN::getOutputs() const {
	return outputs;
}
